...
const Employee = () => {
  // Dynamic state for employee details
  const [employeeName, setEmployeeName] = useState("Chandrappa R");
  const [employmentStartDate, setEmploymentStartDate] = useState("2021-02-08");
  const [employmentEndDate, setEmploymentEndDate] = useState(""); // To be filled dynamically
  const [employmentDuration, setEmploymentDuration] = useState("");
  const [currentDate, setCurrentDate] = useState(new Date().toISOString().slice(0, 10)); // Today's date by default
  ...
  return (
    <div className="p-6 md:p-9 font-serif max-w-lg lg:max-w-3xl mx-auto bg-white shadow-md rounded-lg">
      ...
      <h1 className="text-2xl sm:text-3xl font-bold pt-16">Experience Letter</h1>

      <p className="text-sm md:text-base pt-14 mb-3">To whomsoever it may concern,</p>

      {/* Dynamic Employee Name and Employment Details */}
      <p className="text-sm md:text-base pt-3 mb-3">
        This is to certify that <b>{employeeName}</b> has been an employee at our company, INFINITALENT
        CONSULTING PRIVATE LIMITED, between {employmentStartDate} and{" "}
        {employmentEndDate || <Placeholder label="(End Date)" />} completing{" "}
        {employmentDuration || <Placeholder label="(Duration Of Employment)" />} of employment.
      </p>

      <p className="text-sm md:text-base pt-3 mb-4">
        In this duration, we found {employeeName} to be polite, punctual, dedicated, and performed the roles and
        duties given to them adequately, meeting all of our expectations from them.
      </p>
