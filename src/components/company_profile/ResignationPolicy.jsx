...
const ResignationPolicy = () => {
  ...
  const handleSaveClick = async () => {
    ...
    // Hardcode category to match the backend schema filter
    formData.append("category", "Resignation");
  ...
  const fetchPolicies = async () => {
    try {
      setLoading(true);
      // The API call includes the category filter via query string
      const response = await axios.get(
        `${import.meta.env.VITE_APP_BASE_URL}/company/getAllPolicies?category=Resignation`
      );
      
      // Since the API already filters by category, we take the first result
      const resignationPolicy = response.data.data[0];

      if (resignationPolicy) {
        setPolicy(resignationPolicy);
        setError(null);
      } else {
        setPolicy(null);
  ...
  return (
    <div className="p-4 md:p-6 lg:p-8">
      <div className="w-full mb-4 pb-4 border-b border-gray-300 flex flex-col md:flex-row items-center justify-between">
        <div>
          <h1 className="text-2xl font-medium">Resignation Policy</h1>
          <div className="flex text-sm w-fit text-gray-500 mt-4 gap-1">
            ...
            <span>|</span>
            <span className="cursor-default text-gray-500">Resignation Policy</span>
          </div>
        </div>
