  ...
  const fetchPolicies = async () => {
    try {
      ...
      const response = await axios.get(
        `${import.meta.env.VITE_APP_BASE_URL}/company/getAllPolicies?category=Holiday`
      );
      
      // Since the API already filters by category, we take the first result
      const holidayPolicy = response.data.data[0];

      if (holidayPolicy) {
        setPolicy(holidayPolicy);
        setError(null);
      } else {
        setPolicy(null);
        setError('Holiday Policy not found');
      }
    } catch (err) {
  ...
  return (
    <div className="p-4 md:p-6 lg:p-8">
      <div className="w-full mb-4 pb-4 border-b border-gray-300 flex flex-col md:flex-row items-center justify-between">
        <div>
          <h1 className="text-2xl font-medium">Holiday Policy</h1>
          <div className="flex text-sm w-fit text-gray-500 mt-4 gap-1">
            ...
            <span>|</span>
            <span className="cursor-default text-gray-500">Holiday Policy</span>
          </div>
        </div>
