...
function CreateLetter() {
  ...
  const fetchBusinessUnits = async () => {
    try {
      const response = await axios.get(
        `${import.meta.env.VITE_APP_BASE_URL}//get-bussinessUnits`
      );
    ...
    const fetchSignatories = async () => {
      try {
        ...
        if (response.data.success) {
          const images = {};
          const signatoryObjects = response.data.data.map((signatory) => ({
            id: signatory._id,
            name: signatory.name
          }));
          ...
          setSignatoryOptions(signatoryObjects);
  ...
  const addNewSignatory = async () => {
    if (!newSignatoryName.trim()) {
      toast.error("Signatory name cannot be empty");
      return;
    }
    try {
      ...
      if (response.data.success) {
        const newSignatory = {
          id: response.data.data._id, // or whatever the backend returns as the new signatory's id
          name: response.data.data.name,
        };
        setSignatoryOptions((prev) => [...prev, newSignatory]);
  ...
  const handleSignatureUpload = async (event, signatory) => {
    ...
    setPreviewUrls((prev) => ({
      ...prev,
      [signatory]: previewUrl,
    }));
  };

  const removeSignatory = async (signatory) => {
    try {
      await axios.delete(`${import.meta.env.VITE_APP_BASE_URL}/letter/deleteSignatory/${signatory.id}`);
      setSignatoryOptions((prev) => prev.filter((s) => s.id !== signatory.id)); // Remove from options
      setSignatories((prev) => prev.filter((s) => s !== signatory.name));
      setSignatoryImages((prev) => {
        const updatedImages = { ...prev };
        delete updatedImages[signatory.name];
        return updatedImages;
      });
      setPreviewUrls((prev) => {
        const updatedUrls = { ...prev };
        delete updatedUrls[signatory.name];
        return updatedUrls;
      });
      toast.success(`${signatory.name} removed`);
    } catch (error) {
    ...
    if (categoryName) {
      const fetchTemplatesByCategory = async () => {
        try {
          ...
          // Assuming data only contains enabled templates
          setTemplates(data);
        } catch (error) {
          console.error("Error fetching templates:", error.message);
          setTemplates([]); // Optionally clear templates on error
        }
      };
  ...
  const handleExcludedEmployeesChange = (e) => {
  ...
  const saveCategory = async () => {
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_APP_BASE_URL}/letter/createCategory`,
        {
          name: categoryName,
  ...
  const handleSaveAsTemplateChange = (event) => {
    setSaveAsTemplate(event.target.checked);
  };

  const handleNewTemplateNameChange = (event) => {
  ...
  const saveTemplate = async () => {
    ...
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_APP_BASE_URL}/letter/saveAsTemplate`,
        {
      ...
      toast.error("Template already exists, save with a new template");
    }
  };
  ...
  const updateTemplate = async () => {
    ...
    if (!letterContent || !emailSubject || !emailContent) {
      toast.error("Content, subject and body cannot be empty")
    }

    try {
      const response = await axios.put(
        `${import.meta.env.VITE_APP_BASE_URL}/letter/updateTemplate/${templateId}`,
        {
          letterContent,
          emailSubject,
          emailContent,
      ...
      toast.error("Error updating template. Check server connection")
    }
  };

  const handleSaveAsDraft = async () => {
    // Construct the draft data object
    const draftData = {
      letterName,
      letterContent: {
        content: letterContent, // The letter content coming from the Quill editor or input
        category: categoryName, // The selected category for the letter
        saveAsTemplate: false, // Whether to save as a template (false in this case)
      },
      signatories, // The selected signatories
      audience: {
        group: selectedGroup, // Audience group (e.g., employees, departments)
        options: selectedOptions, // Selected audience options (e.g., specific people, departments)
      },
      status: "draft", // Indicate that this is a draft
      emailDetails: {
        subject: emailSubject, // The email subject
        body: emailContent, // The email content (coming from a Quill editor or input)
      },
    };

    try {
      // Send the draft data to the server via the saveDraft API
      const response = await fetch(
        `${import.meta.env.VITE_APP_BASE_URL}/letter/saveDraft`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
          body: JSON.stringify(draftData), // Convert the draft data to JSON
        }
      );
  ...
  const addExcludedEmployee = (employee) => {
    setExcludedEmployees((prevExcludedEmployees) => {
      // Check if the employee is already in the excluded list
      if (prevExcludedEmployees.some((emp) => emp._id === employee._id)) {
        return prevExcludedEmployees; // Return the current state if already excluded
      }
      // Otherwise, add the employee to the excluded list
  ...
  const handleIncludedEmployeeToggle = (employee) => {
    setExcludedEmployees((prevExcludedEmployees) => {
      // Check if the employee is currently excluded
      if (prevExcludedEmployees.some((emp) => emp._id === employee._id)) {
        // If the employee is in the excluded list, remove them
        return prevExcludedEmployees.filter((emp) => emp._id !== employee._id);
      } else {
        // If the employee is not in the excluded list, add them
        return [...prevExcludedEmployees, employee];
      }
    });
  };
  ...
  return (
    <div className="px-4 py-2 space-y-4">
      <h1 className="text-xl md:text-2xl font-bold">Create Letter</h1>
      <br></br>
      <Link to="/">Home</Link> | <Link to="/app/letter">Letters</Link> |{" "}
      <Link to="/letter/create-letter">Create letter</Link>
      <br></br>
      <button
        className="bg-blue-500 text-white font-semibold px-3 py-1 rounded-sm hover:bg-green-600"
      ...
      {isOverlayVisible && (
        <div className="fixed inset-0 bg-gray-500 bg-opacity-75 flex items-center justify-center z-50">
          <div className="bg-white p-4 rounded-md shadow-md space-y-4 w-80">
            ...
            <input
              ...
              className="border-2 border-gray-300 rounded-sm py-1 pl-2 w-full outline-none focus:border-blue-400"
            />
            <input
              type="text"
              id="description"
            ...
            <div className="space-x-2">
              <button
                className="bg-blue-500 text-white font-semibold px-3 py-1 rounded-sm hover:bg-blue-400"
                onClick={saveCategory}
              >
                Save
              </button>
              <button
                className="bg-gray-300 text-black font-semibold px-3 py-1 rounded-sm hover:bg-gray-400"
                onClick={toggleOverlay}
              >
                Cancel
              </button>
      ...
      <div className="flex flex-col gap-0.5 ">
        ...
        <input
          ...
          className="border-2 border-gray-300 rounded-sm py-1 pl-2 outline-none focus:border-blue-400 w-fit md:w-[18rem]"
        />
      </div>
      <div>
        <div
          ...
          >
            <span
              className={`px-2  border-2 font-bold shadow rounded-[100%] text-gray-500 ${currStep === 1 && "border-blue-400 text-blue-400 "
                }}  ${isFilled[1] === true && " bg-blue-500 text-white"}`}
            >
              1
            </span>
                    ...
                    {filteredEmployee.map((employee) => (
                      ...
                      >
                        <input
                          ...
                          checked={!excludedEmployees.includes(employee)}
                          onChange={() =>
                            handleIncludedEmployeeToggle(employee)
                          }
                          className="w-4 h-4"
                        />
                        <label className="text-sm text-gray-700">
        ...
        <span
          className={`px-2 border-r-2 border-dashed ${isFilled[1] === true && "border-blue-400"
            } `}
        />
        <div
          ...
          >
            <span
              className={`px-2  border-2 font-bold shadow rounded-[100%] text-gray-500 ${currStep === 2 && "border-blue-400 text-blue-400 "
                } ${isFilled[2] === true && " bg-blue-500 text-white"}`}
            >
              2
            </span>{" "}
          ...
          {currStep === 2 && (
            <div className="space-y-3">
              ...
              <select onChange={(e) => insertVariable(e.target.value)}>
                ...
                <option value="{primary_phone_number}">
                ...
                <option value="{date_of_probation}">
                  Probation Completion Date
                </option>
                <option value="{expected_resignation_date}">
                ...
                <option value="{primary_manager}">
                  Primary Manager's Name
                </option>
                <option value="{employment_type}">Employment Type</option>
                <option value="{employee_profile_image}">
                  Employee's Photo
                </option>
                ...
                <option value="{salary_setup_details}">Salary Setup</option>
              </select>
              ...
              <div className="flex gap-1 items-center">
                ...
                {templateId && (
                  <button
                    ...
                    className="bg-green-500 text-white font-semibold px-3 py-1 rounded-sm hover:bg-green-600 active:bg-green-500 w-fit mt-2"
                  >
              ...
              {saveAsTemplate && (
                <div className="flex flex-col gap-2">
                  <input
                    ...
                    className="border-2 border-gray-300 rounded-sm py-1 px-2 outline-none focus:border-blue-400"
                  />
                  <select
                  ...
                  <button
                    ...
                    disabled={!newTemplateName || !templateCategory}
                    className="bg-blue-500 text-white font-semibold px-3 py-1 rounded-sm hover:bg-green-600 active:bg-green-500 disabled:bg-gray-400 disabled:cursor-not-allowed"
                  >
                    Save Template
                  </button>
                </div>
        ...
        <span
          className={`px-2 border-r-2 border-dashed ${isFilled[2] === true && "border-blue-400"
            } `}
        />
        <div
          ...
          >
            <span
              className={`px-2  border-2 font-bold shadow rounded-[100%] text-gray-500 ${currStep === 3 && "border-blue-400 text-blue-400 "
                } ${isFilled[3] === true && " bg-blue-500 text-white"}`}
            >
              3
            </span>
            ...
            <div className="space-y-3">
              <div className="flex flex-col gap-0.5 ">
                ...
                <p className="flex  gap-1 text-sm font-semibold text-gray-500 mt-1">
                  ...
                  This subject will be variable to the employee when the letter
                  is sent.
                </p>
              </div>
              <div className="space-y-1 h-[15rem]">
              ...
              <select onChange={(e) => insertVariable(e.target.value)}>
                <option value="">Insert Variable</option>
                <option value="{letter generation date}">
                  Date of Letter Generation
                </option>
                ...
                <option value="{team}">Team</option>
                <option value="{designation}">Designation</option>
                <option value="{office location}">
                  Reporting Office Location
                </option>
                <option value="{joining date}">Date of Joining</option>
                <option value="{probation}">Probation Completion Date</option>
                <option value="{resignation}">Expected Resignation Date</option>
                <option value="{exit}">Exit Date</option>
                <option value="{experience}">Total Experience</option>
                <option value="{manager}">Primary Manager's Name</option>
                <option value="{emptype}">Employment Type</option>
                <option value="{empphoto}">Employee's Photo</option>
                <option value="{logo}">Company's Logo</option>
                <option value="{registered name">
                  Company's Registered Name
                </option>
                <option value="{salary proration}">Salary Proration</option>
                <option value="{full name}">Full Name</option>
                <option value="{designation apply}">
                  Applied For Designation
                </option>
        ...
        <span
          className={`px-2 border-r-2 border-dashed ${isFilled[3] === true && "border-blue-400"
            } `}
        />
              ...
              <div className="flex flex-col gap-4 mt-6">
                ...
                >
                  {signatoryOptions.map((signatory) => (
                    <option key={signatory.id} value={signatory.name}>
                      {signatory.name}
                    </option>
                  ))}
                </select>
                ...
                {signatoryOptions
                  .filter((signatory) => signatories.includes(signatory.name))
                  .map((signatory) => (
                    <div key={signatory.id} className="flex items-center gap-4 mt-2">
                      <div>
                        <p className="text-sm">{signatory.name}'s Signature</p>
                        <input
                          type="file"
                          accept="image/*"
                          onChange={(e) => handleSignatureUpload(e, signatory.name)}
                          className="block text-sm"
                        />
                      </div>
                      {(previewUrls[signatory.name] || signatoryImages[signatory.name]) && (
                        <img
                          src={previewUrls[signatory.name] || signatoryImages[signatory.name]}
