...
const Export = ({ data }) => {
  const handleExport = () => {
    const formattedData = data.map((report) => ({
      ...
      "UN No.": report.employeeName?.uanNo,
      "Business Unit": report.employeeName?.businessUnit?.name,
      "PF Employee": report.pfEmployee?.toFixed(2),
      "PF Employer": report.pfEmployer?.toFixed(2),
      "Total Accumulation":
        Number(report.pfEmployee?.toFixed(2)) +
        Number(report.pfEmployer?.toFixed(2)),
    }));

    const totalPfEmployee = formattedData.reduce(
      (sum, report) => sum + (Number(report["PF Employee"]) || 0),
      0
    );
    const totalPfEmployer = formattedData.reduce(
      (sum, report) => sum + (Number(report["PF Employer"]) || 0),
      0
    );
    const totalAccumulation = totalPfEmployee + totalPfEmployer;
    ...
    formattedData.push({
      ...
      "UN No.": "",
      "Business Unit": "",
      "PF Employee": totalPfEmployee.toFixed(2),
      "PF Employer": totalPfEmployer.toFixed(2),
  ...
  return (
    <>
      <div className="p-2 md:p-6 lg:p-8">
        <h1 className="text-xl md:text-2xl font-bold mb-4">EPFO Report</h1>
            ...
            <table className="min-w-full bg-gray-100 divide-y divide-gray-200">
              <thead>
                <tr className="text-left text-sm font-medium text-black">
                  ...
                  <th className="px-4 py-2">UAN No.</th>
                  <th className="px-4 py-2">Business Unit</th>
                  <th className="px-4 py-2">PF Employee</th>
                  <th className="px-4 py-2">PF Employer</th>
                ...
                {filteredReports
                  ...
                  .map((report, index) => (
                    <tr key={report._id} className="bg-white hover:bg-gray-50">
                      ...
                      <td className="px-4 py-2 text-sm text-gray-500">
                        {report.employeeName?.uanNo}
                      </td>
                      <td className="px-4 py-2 text-sm text-gray-500">
                        {report.employeeName?.businessUnit?.name}
                      </td>
                      <td className="px-4 py-2 text-sm text-gray-500">
                        {report.pfEmployee?.toFixed(2)}
                      </td>
                      <td className="px-4 py-2 text-sm text-gray-500 truncate">
                        {report.pfEmployer?.toFixed(2)}
                      </td>
                      <td className="px-4 py-2 text-sm text-gray-500">
                        {Number(report.pfEmployee?.toFixed(2)) +
                          Number(report.pfEmployer?.toFixed(2))}
