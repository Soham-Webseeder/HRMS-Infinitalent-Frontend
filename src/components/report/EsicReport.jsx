...
const Export = ({ data }) => {
  const handleExport = () => {
    const formattedData = data.map((report) => ({
      ...
      "ESIC Reg. No.": report.employeeName?.esicNo,
      "Business Unit": report.employeeName?.businessUnit?.name,
      "ESIC Employee": report.esicEmployee?.toFixed(2),
      "ESIC Employer": report.esicEmployer?.toFixed(2),
      "Total Accumulation":
        Number(report.esicEmployee?.toFixed(2)) +
        Number(report.esicEmployer?.toFixed(2)),
    }));

    const totalEsicEmployee = formattedData.reduce(
      (sum, report) => sum + (Number(report["ESIC Employee"]) || 0),
      0
    );
    const totalEsicEmployer = formattedData.reduce(
      (sum, report) => sum + (Number(report["ESIC Employer"]) || 0),
      0
    );
    const totalAccumulation = totalEsicEmployee + totalEsicEmployer;
    ...
    formattedData.push({
      ...
      "ESIC Reg. No.": "",
      "Business Unit": "",
      "ESIC Employee": totalEsicEmployee.toFixed(2),
      "ESIC Employer": totalEsicEmployer.toFixed(2),
      "Total Accumulation": totalAccumulation.toFixed(2),
    });

    const ws = utils.json_to_sheet(formattedData);
    const wb = utils.book_new();
    utils.book_append_sheet(wb, ws, "PF Data");

    writeFile(wb, `esicReport.xlsx`);
  };
...
const EsicReport = () => {
  ...
  const handlePerPageChange = (e) => {
    ...
    setTotalPages(Math.ceil(filteredReports.length / parseInt(e.target.value, 10)));
    setCurrentPage(1);
  };
  ...
  return (
    <>
      <div className="p-2 md:p-6 lg:p-8">
        <h1 className="text-xl md:text-2xl font-bold mb-4">ESIC Report</h1>
            ...
            <table className="min-w-full bg-gray-100 divide-y divide-gray-200">
              <thead>
                <tr className="text-left text-sm font-medium text-black">
                  ...
                  <th className="px-4 py-2">ESIC Reg. No.</th>
                  <th className="px-4 py-2">Business Unit</th>
                  <th className="px-4 py-2">ESIC Employee</th>
                  <th className="px-4 py-2">ESIC Employer</th>
                  <th className="px-4 py-2">Total Accumulation</th>
                </tr>
              </thead>
              <tbody>
                {filteredReports
                  ...
                  .map((report, index) => (
                    <tr key={report._id} className="bg-white hover:bg-gray-50">
                      ...
                      <td className="px-4 py-2 text-sm text-gray-500">
                        {report.employeeName?.esicNo}
                      </td>
                      <td className="px-4 py-2 text-sm text-gray-500">
                        {report.employeeName?.businessUnit?.name}
                      </td>
                      <td className="px-4 py-2 text-sm text-gray-500">
                        {report.esicEmployee?.toFixed(2)}
                      </td>
                      <td className="px-4 py-2 text-sm text-gray-500 truncate">
                        {report.esicEmployer?.toFixed(2)}
                      </td>
                      <td className="px-4 py-2 text-sm text-gray-500">
                        {Number(report.esicEmployee?.toFixed(2)) +
                          Number(report.esicEmployer?.toFixed(2))}
