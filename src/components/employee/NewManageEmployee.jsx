...
export default function NewManageEmployee() {
  ...
  const fetchEmployees = async () => {
    try {
      const response = await axios.get(
        `${import.meta.env.VITE_APP_BASE_URL}/employee/getAllEmployeesWithRole`,
        {
          params: {
            page: currentPage,
            limit: perPage,
                  ...
                  {employees.map((data, index) => (
                    <tr key={data._id} className="text-gray-600 text-left">
                      ...
                      <td className="px-2 py-1">
                        {data.photograph ? (
                          <img
                            src={`${import.meta.env.VITE_APP_IMG_URL}/${data.photograph}`}
                            alt={`${data.firstName || 'Employee'} Photograph`}
                      ...
                      <td className="px-2 py-1">
                        {data.employeeType}-{data.empId}
                      </td>

                      <td className="px-2 py-1">{data.firstName}</td>
                      <td className="px-2 py-1">{data.lastName}</td>
